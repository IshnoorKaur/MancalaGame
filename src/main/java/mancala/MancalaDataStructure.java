...
public class MancalaDataStructure implements Serializable {
    ...
    public int getStoreCount(final int playerNum) {
        final Countable storeC = data.get(storePos(playerNum));
        return storeC.getStoneCount();
    }
    ...
    public int getNumStones(final int pitNum) {
        final Countable inPit = data.get(pitPos(pitNum));
        return inPit.getStoneCount();
    }
    ...
    public void setUpPits() {
        for (int i = 0; i < PLAYER_ONE; i++) {
            data.get(i).addStones(startStns);
        }
    ...
    /* default */int positionOfIterator() {
        return iteratorPos;
    }
}
